...
My examination of the attribute estimates from an ANN resulted in similar findings.As it is seen in Figure \ref{AHM_simu}I observed noticeable variation in attribute estimates in both simulated data sets after ensemble averaging, particularly for response patterns with HCI values lower than 0.7.
